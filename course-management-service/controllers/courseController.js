...
async function searchCourse(req, res) {
  ...
  try {
    const results = await Course.find({
      crsname: { $regex: new RegExp(query, 'i') },
    });
...
async function getAllCourses(req, res) {
  ...
  if (role === 'admin') {
    query = Course.find().select(
      'crscode crsname instructorId description status remarks price date'
    );
  } else {
    query = Course.find({ status: 'approved' }).select(
      'crscode crsname instructorId description status remarks price date'
    );
  }
  ...
  if (role === 'instructor') {
    ...
    try {
      ...
      if (!courses || courses.length === 0) {
        return res
          .status(404)
          .json({ error: 'No courses found for the current instructor' });
      }
      res.json(courses);
    } catch (err) {
      console.error(err);
...
async function getCoursesByArray(req, res) {
  ...
  try {
    ...
    if (courses.length > 0) {
      ...
      return res
        .status(400)
        .json({ error: `No courses with the passed cscodes` });
    }
  } catch (err) {
    console.error(err);
    res
...
async function updateCourse(req, res) {
  ...
  if (role === 'instructor') {
    ...
    if (courseDetails) {
      ...
      return res
        .status(404)
        .json({ error: 'Course with provided code not found' });
    }
      ...
      if (courseDetails) {
        ...
        try {
          ...
          if (updatedItem) {
            return res
              .status(200)
              .json({ status: 'Item updated', updatedItem });
          } else {
            return res
              .status(404)
              .json({ error: 'Course with provided code not found' });
    ...
    if (instructorId === currentUserId) {
      try {
        ...
        if (deletedItem) {
          ...
          return res
            .status(404)
            .json({ error: 'Course with the entered code not found' });
        }
      } catch (err) {
        console.log(err);
        return res.status(500).json({ error: 'Internal server error' });
